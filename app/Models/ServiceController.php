...
class ServiceController extends Controller {
	...
	*/
	...
	{
		$event 			= Commaned::find($id);
		$event->status 	= 1; // Volviendo a solicitar
		$event->save();
        ...
        $dat_s = array(
            ...
            'status' 		=> $event->status,
            'change_from'   => 'user_admin'
        );

        $fb_server->orderStatus($dat_s);

		/**
		 * VOLVEMOS A SOLICITAR CONDUCTORES
		 */
		...
		return redirect(env('admin').'/Services')->with('message','Solicitud enviada correctamente.');
	}
}
